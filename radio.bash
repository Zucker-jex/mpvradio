...
if [ ! -f "$LIST_FILE" ]; then
    ...
    echo "[Vocaloid Radio]http://curiosity.shoutca.st:8019/stream" >> "$LIST_FILE"
fi
...
while true; do
    ...
    case "$key" in
        $'\x1B')
            read -rsn2 key
            if [[ "$key" == "[A" || "$key" == "OA" || "$key" == "k" ]]; then # 向上箭头或k
                ((selected--))
                if [ "$selected" -lt 0 ]; then
                    selected=$((${#radios[@]} - 1))
                fi
            elif [[ "$key" == "[B" || "$key" == "OB" || "$key" == "j" ]]; then # 向下箭头或j
                ((selected++))
